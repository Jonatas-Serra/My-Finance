...
export default function Payables() {
  ...
  const { payables, loading, handleDeleteAccount, getAccounts, PayAccount, UnpayAccount } = useAccounts();
  ...
  const filteredAccounts = payables.filter(payable => 
    payable.description?.toLocaleLowerCase().includes(searchTerm.toLocaleLowerCase()) ||
    ...
    <>
      ...
      >
        <ContentModalPaid>
          ...
          <p>Tem certeza que deseja desfazer o pagamento da conta <strong>{selectedAccount.description}</strong> ?</p>
          <div>
            <button
              onClick={() => {
      ...
      >
        <ContentModalPaid>
          ...
          <p>Tem certeza que deseja fazer o pagamento da conta <strong>{selectedAccount.description}</strong> ?</p>
          <div className='flex'>
            <div className='block'>
          ...
          <div>
            ...
            <button
              onClick={() => {
                setIsOpenCheck(false);
                handlePayAccount(selectedAccount._id, walletId, new Date(payday));
              }}
            >
              Pagar
            </button>
      ...
      >
        <ContentModalDelete>
          ...
          <p>Tem certeza que deseja excluir a conta <strong>{selectedAccount.description}</strong>?</p>
          <div>
            <button
              onClick={() => {
                setIsOpenDel(false);
              ...
              <tbody>
                {paginatedAccounts.map(payable => (
                  <tr key={payable._id}>
                    ...
                    <td
                      className={
                        payable.status === 'Late' ? 'late' :
                        payable.status === 'Paid' ? 'paid' :
                        payable.status === 'Pending' ? 'pending' : ''
                      }
                    >{
                      payable.status === 'Late' ? 'Conta atrasada' :
                      payable.status === 'Paid' ? 'Conta paga' :
                      payable.status === 'Pending' ? 'A vencer' : 'Desconhecido'
                    }
                    </td>
                    <td>
                      <a 
                      ...
                      >
                        <div className='tooltip'>
                          {payable.isPaid ? <FiCheckSquare size={20} /> : <FiSquare color='#181d29' size={20} />}
                          <span className='tooltiptext'>{payable.isPaid ? 'Desfazer recebimento' : 'Receber conta'}</span>
                        </div>
                      </a>
                    </td>
                  </tr>
